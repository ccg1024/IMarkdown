...
import { defaultAppDirectory, closeMessage, copyProp } from './window/tools'
...
export function hasCache(filepath: string): boolean {
  return Object.prototype.hasOwnProperty.call(fileCache, filepath)
}
// NOTE: just readonly
  ...
  if (filepath) {
    ...
    if (fileData) {
      ...
      if (headInfo) {
        ...
        for (key in headInfo) {
          copyProp(_headInfo_, headInfo, key)
        }
      }
      if (rest) {
        let key: keyof Omit<FileData, 'headInfo'>
        for (key in rest) {
          copyProp(_rest_, rest, key)
        }
      }
      // mount update
